      ...
      <div className="bg-gray-50 border-b border-gray-100 py-12 md:py-20">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 text-center">
          <h1 className="text-4xl md:text-6xl font-bold tracking-tighter mb-4">
            {categoryFilter ? categoryFilter.toUpperCase() : originFilter === 'international' ? 'IMPORTED PREMIUM' : originFilter === 'domestic' ? 'LOCAL HERITAGE' : 'ALL COLLECTIONS'}
          </h1>
        ...
        <div className="flex flex-col lg:flex-row gap-12">
          ...
          <aside className="w-full lg:w-64 space-y-10">
                  ...
                  {['all', 'international', 'domestic'].map((origin) => (
                    <button
                      ...
                      className={`text-sm text-left font-medium transition-colors ${
                        (origin === 'all' && !originFilter) || originFilter === origin
                          ? 'text-black'
                          : 'text-gray-400 hover:text-black'
                      }`}
                    >
                      {origin === 'all' ? 'All Origins' : origin === 'international' ? 'Imported Premium' : 'Local Heritage'}
                    </button>
                  ))}
                </div>
              </div>
            ...
            <div className="space-y-4 pt-10 border-t border-gray-100">
              ...
              <div className="flex flex-col space-y-2">
                {['Men', 'Women', 'Kids', 'Home', 'Electronics'].map((cat) => (
                  <button
                    key={cat}
                    onClick={() => {
